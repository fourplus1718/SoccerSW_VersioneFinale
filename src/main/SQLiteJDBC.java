...
public class SQLiteJDBC {
	...
	private static void controlTableName(String tableName, ExtendedFixture f, HSSFSheet sheet, Statement stmt, int year)
	{
		...
		if (tableName.equals("BASICS")) {
			score = XlSUtils.basic2(f, sheet, var6, var3, var1);
			...
			score = XlSUtils.halfTimeOnly(f, sheet, var1);
		} else if (tableName.equals("HALFTIME2")) {
			score = XlSUtils.halfTimeOnly(f, sheet, var2);
			...
			try {
				for (FinalEntry f : finals) {
					...
					String sql = "INSERT INTO FINALS "
							...
							+ (float) Math.round(f.prediction * var100000) / var100000 + COMMA + f.threshold + COMMA + f.lower + COMMA
	...
	private static ExtendedFixture createExtendedFixture(String date, String homeTeamName, String awayTeamName,
		...
		float var0 = 0f;
		synchronized(format){
			return new ExtendedFixture(format.parse(date), homeTeamName, awayTeamName,new Result(homeGoals, awayGoals), competition).withMatchday(matchday).withOdds(var0, var0, over, under).withYear(year);
		}
	}
			...
			try {
				for (HTEntry f : halftimeData) {
					...
					String sql = "INSERT INTO HALFTIMEDATA "
							...
							+ COMMA + (float) Math.round(f.fe.prediction * var100000) / var100000 + COMMA + f.fe.threshold + COMMA
